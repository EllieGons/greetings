...
updateGreetingCardAndLink();
    ...
    // Function to apply changes to the widget based on customizations
    function applyChangesToWidget() {
        var userName = document.getElementById('user-name').value || 'Muffin'; // Get user name value
        var color = document.querySelector(".selected-color .color-preview").style.backgroundColor;
        var link = generateUniqueLink(userName, color);

        // Redirect to the widget page with updated parameters
        window.location.href = link;
    }

    // Add event listener for input change to apply changes to the widget
    document.getElementById('user-name').addEventListener('input', applyChangesToWidget);
    document.querySelector(".selected-color").addEventListener('click', applyChangesToWidget);
});
